...
1. Resolve the current `PaperMC/Velocity@dev/3.0.0` head to a full SHA. Compare it against the newest reviewed head recorded in `VELOCITY_SYNC.md`; if that head is not pinned, compare from the verified sync commit and use the prior review to avoid repeating its decisions.
