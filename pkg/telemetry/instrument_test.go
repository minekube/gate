...
func (m *simpleEventMgr) UnsubscribeAll(events ...event.Event) int { return 0 }
func (m *simpleEventMgr) HasSubscriber(events ...event.Event) bool { return false }
func (m *simpleEventMgr) FireParallel(e event.Event, handlers ...event.HandlerFunc) { m.Fire(e) }
func (m *simpleEventMgr) FireAsync(e event.Event) { m.Fire(e) }
func (m *simpleEventMgr) FireAsyncParallel(e event.Event, handlers ...event.HandlerFunc) { m.Fire(e) }
...
func TestInstrumentProxyTelemetry(t *testing.T) {
    // Initialize telemetry with explicit configuration and stdout tracer
    cfg := &config.Config{
...
func TestTelemetryValidation(t *testing.T) {
    tests := []struct {
        ...
        {
            name: "missing metrics endpoint when enabled",
            config: &config.Config{
                Telemetry: config.Telemetry{
                    Metrics: config.TelemetryMetrics{
                        Enabled:  true,
            ...
            wantErr: true,
            errMsg:  "Telemetry metrics endpoint cannot be empty",
        },
        {
            ...
            wantErr: true,
            errMsg:  "Invalid telemetry metrics exporter",
        },
        {
            name: "missing prometheus path",
            config: &config.Config{
                Telemetry: config.Telemetry{
                    Metrics: config.TelemetryMetrics{
                        Enabled:  true,
            ...
            wantErr: true,
            errMsg:  "Prometheus metrics path cannot be empty",
        },
        {
            ...
            wantErr: true,
            errMsg:  "Telemetry tracing endpoint cannot be empty",
        },
        {
            ...
            wantErr: true,
            errMsg:  "Invalid telemetry tracing exporter",
        },
    }

    for _, tt := range tests {
        t.Run(tt.name, func(t *testing.T) {
            _, errs := tt.config.Validate()
            if tt.wantErr {
...
// TestTelemetryInitialization verifies telemetry is properly initialized and can be retrieved
func TestTelemetryInitialization(t *testing.T) {
    // Create config with both metrics and tracing enabled
    cfg := &config.Config{
        Telemetry: config.Telemetry{
            Metrics: config.TelemetryMetrics{
                Enabled: true,
    ...
    // Verify telemetry instance is created and properly configured
    assert.NotNil(t, tel, "Telemetry instance should be created")

    // Test with metrics only
    ...
    metricsTel, metricsCleanup, err := New(context.Background(), metricsCfg)
    assert.NoError(t, err)
    defer metricsCleanup()
    assert.NotNil(t, metricsTel, "Telemetry instance should be created with metrics only")

    // Test with tracing only
    ...
    tracingTel, tracingCleanup, err := New(context.Background(), tracingCfg)
    assert.NoError(t, err)
    defer tracingCleanup()
    assert.NotNil(t, tracingTel, "Telemetry instance should be created with tracing only")

    // Test with neither metrics nor tracing
    emptyCfg := &config.Config{
        Telemetry: config.Telemetry{},
    }
    emptyTel, emptyCleanup, err := New(context.Background(), emptyCfg)
    assert.NoError(t, err)
    defer emptyCleanup()
    assert.NotNil(t, emptyTel, "Telemetry instance should be created even with empty config")
}