...
fn main() {
    let contract = PathBuf::from("../../../wasm/wit/contract.json");
    println!("cargo:rerun-if-changed={}", contract.display());
    let document = fs::read_to_string(&contract).expect("read Gate contract");
    ...
    fs::write(output, generated).expect("write Gate contract constants");
}
